...
const LABELS = {
	...
	captions: [
		...
		"③ 그래서 Parquet는 덩어리마다 블룸 필터를 함께 저장한다",
	...
	aria: "Parquet 파일에서 블룸 필터가 왜 필요한지 반복 재생하는 애니메이션. message_id가 M-42인 행을 찾는데, 파일 안 덩어리 넷의 min/max가 전부 겹쳐 목차만으로는 하나도 거를 수 없어 넷을 다 읽어야 할 판이다. 그래서 Parquet는 덩어리마다 작은 블룸 필터를 함께 저장한다. 조회 값을 해시해 나온 0번과 5번 자리를 네 덩어리의 블룸 필터에 각각 물어보면, 어느 한 자리가 0인 세 덩어리는 확실히 없음이라 건너뛰고, 두 자리가 모두 1인 덩어리 하나만 읽는다.",
},
	en: {
		captions: [
