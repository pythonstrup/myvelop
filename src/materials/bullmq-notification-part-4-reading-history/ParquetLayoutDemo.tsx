...
const LABELS = {
	ko: {
		...
		aria: "Parquet 파일 하나의 내부 배치를 따라가는 반복 애니메이션. 열로만 모으면 파일 전체가 열 넷으로 이어지지만, Parquet는 먼저 행을 덩어리(row group) 셋으로 자르고 그 덩어리 안에서 다시 열별로 모아 둔다. 파일 꼬리에는 덩어리마다 어느 위치에 있고 열별 최소·최대가 얼마인지가 적힌다. 그래서 읽는 쪽은 꼬리를 먼저 읽고, 조건에 맞는 덩어리 하나와 필요한 열 하나의 자리로 곧장 건너뛴다.",
	},
	en: {
		captions: [
