...
const LABELS = {
	ko: {
		...
		aria: "같은 하루치 데이터를 두 가지 모양으로 S3에서 읽을 때의 차이를 반복 재생하는 애니메이션. 위 레인은 큰 파일 하나여서 왕복 한 번으로 100MB를 읽어온다. 아래 레인은 같은 100MB가 작은 객체 1만 2천 개로 나뉘어 있어 객체마다 HTTP 왕복이 붙고, 총 바이트가 같은데도 왕복 횟수가 1만 2천 번으로 늘어 조회가 느려진다. 파일을 합쳐야 풀리는 문제이며 포맷을 JSON에서 Parquet로 바꿔도 왕복 횟수는 그대로다.",
	},
	en: {
		captions: [
