...
socialImage: "../../../assets/blog/dissecting-parquet/og.png"
---
import BloomFalsePositiveDemo from "@/materials/dissecting-parquet/BloomFalsePositiveDemo";
import BloomFilterDemo from "@/materials/dissecting-parquet/BloomFilterDemo";
import BloomPruningDemo from "@/materials/dissecting-parquet/BloomPruningDemo";
import FileLayoutDemo from "@/materials/dissecting-parquet/FileLayoutDemo";
import FooterTocDemo from "@/materials/dissecting-parquet/FooterTocDemo";
import RangeReadDemo from "@/materials/dissecting-parquet/RangeReadDemo";
import StorageOrderDemo from "@/materials/dissecting-parquet/StorageOrderDemo";
import UnitHierarchyDiagram from "@/materials/dissecting-parquet/UnitHierarchyDiagram";
...
Everything in Parquet's design serves reading less. Columns gather so only the needed columns are read; row groups cut so only the needed groups are read; pages cut so only the needed pages are decompressed. The footer at the tail says where to read, and Bloom filters prune where min/max cannot. Even with the file on S3, Range fetches just the slices. The comparison with JSONL comes in [the next post](/blog/querying-s3-without-parquet/).
