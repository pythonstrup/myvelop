...
[The last post](/blog/27/) dissected a Parquet file. After opening up the footer-at-the-end promise, the per-row-group min/max statistics and the Bloom filters, a question in the opposite direction remained. Is there a way to not use Parquet? Can the JSONL we already accumulate be queried efficiently as it is?
