...
socialImage: "../../../assets/blog/bullmq-notification-part-3-delivery-history/og.png"
---
## Introduction

[Part 1](/blog/bullmq-notification-part-1-what-is-bullmq) and [Part 2](/blog/bullmq-notification-part-2-rate-limit) were about getting messages out reliably. This post is about what comes after they are sent.
...
![A message history model that publishes events only after the result is finalized. When a response arrives, every message in the request is assigned a terminal status; only when no response was received at all is ERROR recorded after retries are exhausted.](../../../assets/blog/bullmq-notification-part-3-delivery-history/event/lifecycle.svg)
...
>
> ![Transactional outbox pattern: the application writes the business table and the outbox table in one local transaction, and a separate relay reads outbox rows by polling or CDC and delivers them to the analysis platform at least once.](../../../assets/blog/bullmq-notification-part-3-delivery-history/history/outbox.svg)
>
...
![A serial pipeline where the worker writes events straight to S3 with PutObject, and s3:ObjectCreated notifications flow through SQS with a DLQ to Lambda, which loads the analysis platform.](../../../assets/blog/bullmq-notification-part-3-delivery-history/history/direct-s3.svg)
...
![A structure where the worker publishes to SNS, which fans out independently to an S3 archive path through Firehose and an analysis-platform loading path through SQS.](../../../assets/blog/bullmq-notification-part-3-delivery-history/history/sns.svg)
...
![A structure where single-message sends go through the Firehose buffer while bulk sends bypass Firehose and load S3 directly, splitting the path in two.](../../../assets/blog/bullmq-notification-part-3-delivery-history/history/firehose-s3.svg)
...
![Third-party webhooks entering through the shared ALB's IP target group straight to worker pods in the cluster, where the pod validates and filters, then loads into the history S3 through a buffer.](../../../assets/blog/bullmq-notification-part-3-delivery-history/webhook-receiver/alb-application.svg)
...
![Third-party webhooks entering the existing shared ALB and forwarding to a Lambda target group, where Lambda validates and filters, then loads into the history S3 through a buffer. Traffic never enters the EKS cluster.](../../../assets/blog/bullmq-notification-part-3-delivery-history/webhook-receiver/alb-lambda.svg)
...
![Third-party webhooks entering an API Gateway HTTP API, stacking straight into a queue through the SQS integration; Lambda pulls batches, validates and filters, then loads into the history S3, joining the existing pipeline.](../../../assets/blog/bullmq-notification-part-3-delivery-history/webhook-receiver/apigw.svg)
