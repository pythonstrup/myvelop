...
tags: ["BullMQ","parquet","aws","architecture","s3","duckdb","athena"]
...
---
import ColumnarLayoutDemo from "@/materials/bullmq-notification-part-4-reading-history/ColumnarLayoutDemo";
import ParquetLayoutDemo from "@/materials/bullmq-notification-part-4-reading-history/ParquetLayoutDemo";
import RangeReadDemo from "@/materials/bullmq-notification-part-4-reading-history/RangeReadDemo";
import RowGroupPruningDemo from "@/materials/bullmq-notification-part-4-reading-history/RowGroupPruningDemo";
import SmallFilesDemo from "@/materials/bullmq-notification-part-4-reading-history/SmallFilesDemo";

## Introduction

[Part 3](/blog/bullmq-notification-part-3-delivery-history) was about where to store delivery history.
...
![The worker loads JSON per job into the history S3 exactly as in Part 3, but the object-creation event and the SQS-Lambda chain are gone and a k8s CronJob stands in their place. Once a day the CronJob reads a finished day's partition whole and writes it as one Parquet file, and the engine reads that Parquet together with today's JSON.](../../../assets/blog/bullmq-notification-part-4-reading-history/parquet-architecture.svg)
...
![The worker loads each job's delivery result into S3 as JSON immediately, and a nightly batch converts the previous day into a single daily Parquet file. Queries read the Parquet through yesterday together with today's JSON.](../../../assets/blog/bullmq-notification-part-4-reading-history/parquet-pipeline.svg)
