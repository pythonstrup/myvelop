...
socialImage: "../../../assets/blog/passwordless-db-access-least-privilege/og.png"
---
import PasswordlessAuthDemo from "@/materials/passwordless-db-access-least-privilege/PasswordlessAuthDemo";
import PrivilegesTimingDemo from "@/materials/passwordless-db-access-least-privilege/PrivilegesTimingDemo";
import TrustChainDiagram from "@/materials/passwordless-db-access-least-privilege/TrustChainDiagram";
import PrivilegesOrderDiagram from "@/materials/passwordless-db-access-least-privilege/PrivilegesOrderDiagram";
...
![IRSA 동작 원리 — ServiceAccount 애노테이션에서 Pod Identity Webhook, 프로젝티드 토큰, STS AssumeRoleWithWebIdentity를 거쳐 임시 자격증명을 발급받는 흐름](../../../assets/blog/passwordless-db-access-least-privilege/irsa.svg)
...
![관리자·소유자·앱 계정의 권한 경계 — 관리자는 시크릿 매니저 뒤에서 사람만 쓰고, 소유자는 NOLOGIN이라 접속 자격증명이 없으며 SET ROLE로만 도달한다. 애플리케이션 파드에 로드되는 것은 DML만 가능한 앱 계정 경로뿐이다](../../../assets/blog/passwordless-db-access-least-privilege/roles.svg)
