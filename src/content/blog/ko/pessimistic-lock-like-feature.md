...
tags: ["java","redis","RedisLockRegistry","Spring","동시성 제어","비관적 락","MySQL"]
series: "동시성 제어"
---
...
![동시성 제어를 하지 않으면 중복 요청이 전부 적용된다.](../../../assets/blog/pessimistic-lock-like-feature/01.png)
...
이전에 ["UPDATE 한 줄로 끝내는 동시성 문제"](/ko/blog/concurrency-single-update/) 글에서는 명시적인 읽기 락 없이 동시성을 제어하는 방법을 다뤘다. 하지만 이번 요구사항에서는 <strong>존재하지 않는 데이터를 새로 만들어야 하기 때문에</strong> 배타락이나 낙관적 락을 사용할 수 없다.
...
![인덱스가 없을 때의 Next Key Lock](../../../assets/blog/pessimistic-lock-like-feature/02.png)
...
![좋아요 기능으로 알아보는 비관적 락 이미지 3](../../../assets/blog/pessimistic-lock-like-feature/03.png)

user\_100이 review\_id=1 조건으로 SELECT FOR UPDATE를 실행했다고 가정해보자.

![(review_id) index 샘플 데이터](../../../assets/blog/pessimistic-lock-like-feature/04.png)

*(review_id) 인덱스 샘플 데이터*

MySQL은 review\_id=1 인덱스 범위에 Next-Key Lock을 건다.

![(review_id) index Locking](../../../assets/blog/pessimistic-lock-like-feature/05.png)
...
![member_id에만 인덱스를 건 경우](../../../assets/blog/pessimistic-lock-like-feature/06.png)

![(member_id) index 데이터 샘플](../../../assets/blog/pessimistic-lock-like-feature/07.png)

*(member_id) 인덱스 샘플 데이터*

하나의 멤버가 여러 리뷰에 대해 좋아요를 동시에 누르면 병목 발생하게 된다.

![(member_id) index Locking](../../../assets/blog/pessimistic-lock-like-feature/08.png)

### Case04. (review\_id, member\_id) 인덱스

가장 작은 락 범위를 가질 가능성이 높은 방법이다.

![(review_id, member_id) INDEX 시나리오](../../../assets/blog/pessimistic-lock-like-feature/09.png)

예를 들어, 인덱스 데이터가 다음과 같이 구성되어 있다면

![(review_id, member_id) index 데이터 샘플](../../../assets/blog/pessimistic-lock-like-feature/10.png)

*(review_id, member_id) 인덱스 샘플 데이터*

review\_id=1, member\_id=150 조건으로 조회 시, member\_id가 93\~234 범위에 Gap Lock이 걸린다.

![(review_id, member_id) index Locking](../../../assets/blog/pessimistic-lock-like-feature/11.png)
...
Redis 분산 락에 대해 알아보고 싶다면, ["Lettuce 분산 락의 오해와 진실"](/ko/blog/lettuce-distributed-lock-myths/) 포스트를 추천한다.

## 동시성 처리 시리즈

-   [처음부터 다시 배우는 Java 동시성 제어](/ko/blog/java-concurrency-control-from-scratch/)
-   [UPDATE 한 줄로 끝내는 동시성 처리](/ko/blog/concurrency-single-update/) 
-   좋아요 기능으로 알아보는 넥스트 키 락
-   [Lettuce 분산 락의 오해와 진실](/ko/blog/lettuce-distributed-lock-myths/)
-   [AOP로 동시성 처리 코드 분리하기](/ko/blog/concurrency-control-aop/)
