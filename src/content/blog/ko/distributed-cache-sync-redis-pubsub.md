...
![분산 시스템에서의 캐시](../../../assets/blog/distributed-cache-sync-redis-pubsub/01.png)

이때 ServerA에서 리뷰를 수정했다고 해보자.

![분산 시스템에서의 캐시 - 캐시 업데이트로 인한 데이터 부정합 발생](../../../assets/blog/distributed-cache-sync-redis-pubsub/02.png)
...
![L1/L2 캐시가 모두 성공하면 로컬 캐시를 조회한다.](../../../assets/blog/distributed-cache-sync-redis-pubsub/03.png)

#### GET을 시도했을 때, L2 캐시만 존재하는 경우

![L2 캐시만 존재하면 레디스를 조회한다. 대신 backfill을 통해 로컬 캐시를 채워준다.](../../../assets/blog/distributed-cache-sync-redis-pubsub/04.png)

#### EVICT를 시도하면 모든 캐시 계층에 접근하여 삭제

![커스텀 캐시 매니저는 2개의 캐시를 모두 반환하기 때문에 Eviction이 일어날 때 두 계층의 캐시를 모두 삭제한다.](../../../assets/blog/distributed-cache-sync-redis-pubsub/05.png)
...
![메시지 발행 플로우](../../../assets/blog/distributed-cache-sync-redis-pubsub/06.png)

#### 메시지 구독

![메시지 구독자는 메시지를 받아 캐시를 제거하게 된다.](../../../assets/blog/distributed-cache-sync-redis-pubsub/07.png)
